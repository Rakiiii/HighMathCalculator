...
class PolynomialPresenter : MvpPresenter<PolynomialViewInterface>(), LifecycleObserver
{
    ...
    private val errorHandler = CoroutineExceptionHandler(handler = { _, error ->
        ...
        {
            //if exception with custom time t=when we must stop calculation animation for that calc
            uiScope.launch {
        ...
        when (error)
        {
            ...
            is WrongDiofantPolynomialVariableLengthException ->
            {
                presenterScope.launch(Dispatchers.Default) {
                    ...
                    val errorBitmap =
                        mExceptionRenderModel.drawErroredPolynomialWithWrongSubstring(
                            presenterScope,
                            error.input,
                            error.unrecognizablePart
                        )
    ...
    fun onClickPolynomial(polynomial: String)
    {
        presenterScope.launch(Dispatchers.Default) {
            ...
            var polynomialSize =
                mPaint.getMultiLinePolynomialSize(initedPolynomial, range)
    ...
    //add calc animatoin, and if something goes wrong it must be removed
    ...
    )
    {
        ...
        val job = presenterScope.launch(Dispatchers.Default + errorHandler)
        {
            ...
            if ((jobMap[time.timeInMillis]
                    ?: return@launch).isCancelled
            ) return@launch
 ...
 */
    ...
    fun onLoadSavedInstance()
    {
        presenterScope.launch(Dispatchers.IO + errorHandler) {
            ...
            val result =
                mPolynomialDataBaseModel.selectAll().sortedBy { s -> s.time }
                    .reversed()
                    .toMutableList()
            delay(1000)
            uiScope.launch {
